...
def main():
    ...
    cinfo_topic = rospy.get_param('~cinfo_topic_name')
    ...
    gt_positions, gt_times = load_csv_data(gt_out_fname)
    ...
    # else:
    rospy.loginfo("Loading data from rosbags {:s} and {:s}".format(loc_bag_fname, cinfo_bag_fname))
    # if msgs is None:
    skip_time = 20
    skip_time_end = 60
    # rospy.loginfo("Input file not valid, loading from rosbag")
    cinfo = None
    if len(cinfo_bag_fname) > 0:
        cinfo = load_rosbag_cinfo(cinfo_bag_fname, cinfo_topic)
    loc_msgs = load_rosbag_msgs(loc_bag_fname, loc_topic_name, skip_time=skip_time, skip_time_end=skip_time_end)

    if loc_msgs is None or gt_positions is None:
        exit(1)

    rospy.loginfo("Loaded {:d} localization messages".format(len(loc_msgs)))
    msgs = process_msgs(loc_msgs, cinfo, np.array([0, 0, 0]))
    # msgs = process_msgs(loc_msgs, cinfo, gt_positions[0, :] + np.array([1, -1.5, 0]))
    loc_positions = msgs_to_pos(msgs)
    ...
    loc_x = np.array(loc_positions[:, 0]).flatten()
    loc_y = np.array(loc_positions[:, 1]).flatten()
    loc_z = np.array(loc_positions[:, 2]).flatten()
